...
const EditButtons = (props: EditButtonsProps) => {
  ...
  const parseDebtToText = (): string => {
    const parsedText = [
      ...
      `\nItems:\n${state[props.debtId].items
        .map(
          (debtItem) =>
            `${debtItem.description}: ${debtItem.price.toFixed(2)} ${
              state[props.debtId].currency
            }`,
        )
        .join("\n")}`,
      `\nTotal amount: ${calculateTotalDebt(state[props.debtId]).toFixed(2)} ${
        state[props.debtId].currency
      }`,
      `Per person: ${calculateUserDebt(state[props.debtId]).toFixed(2)} ${
        state[props.debtId].currency
      }`,
  ...
  return (
    <View style={{ position: "relative", flexDirection: "column", padding: 5 }}>
      <View style={{ flexDirection: "row", justifyContent: "flex-end" }}>
        <CustomButton
          onPressAlertText="Copied!"
          style={styles.editButton}
          onPress={copyDebtToClipboard}>
...
const Prices = (props: PricesProps) => {
  ...
  return (
    <View style={{ backgroundColor: Colors.darkestBlue, padding: 5 }}>
      ...
      {props.editActive && (
        <EditDebtInput
          onBlur={() => props.setEditActive(false)}
          debtName={state[props.debtId].description}
...
const PickerSwiper = (props: { debtId: string }) => {
  ...
  const getPickerEntries = () => {
    return Object.entries(debtHoldersState).map(
      ([debtHolderId, debtHolder]) => {
        return {
          id: debtHolderId,
  ...
  const onScrollEnd = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    // Scroll to right
    if (
      (xOffset >= screenWidth * 0.3 && page === 0) ||
      xOffset > screenWidth * 0.7
    ) {
