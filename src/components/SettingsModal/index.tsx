...
const validateMobilePay = (mobilePay: string) => {
  return (
    !isNaN(parseInt(removeSpaces(mobilePay))) &&
    parseInt(removeSpaces(mobilePay)).toString().length === 9
  );
};

const splitToChunks = (value: string, chunkSize: number) =>
  value
    .split("")
...
const parseMobilePay = (mobilePay: string) => {
  ...
  for (let i = formattedMobilePay.length - 1; i > 2; i--) {
    formattedMobilePay[2] = [
      ...formattedMobilePay[2],
      ...formattedMobilePay[i],
    ].join("");
...
const parseBankAccount = (bankAccount: string) => {
  const formattedBankAccount: string | string[] = removeSpaces(
...
export const SettingsModal = (props: SettingsModalProps) => {
  ...
  const [validBankAccount, isValidBankAccount] = useState(
    validateBankAccount(state.bankAccount),
  );
  const [validMobilePay, isValidMobilePay] = useState(
    validateMobilePay(state.mobilePay),
  );
  ...
  return (
    <CustomModal setModal={props.setModal} title="Settings">
      ...
      <View style={styles.inputContainer}>
        ...
        <CustomInput
          ...
          value={bankAccount}
          onChange={(e: NativeSyntheticEvent<TextInputFocusEventData>) =>
            setBankAccount(e.nativeEvent.text)
          }
          onBlur={formatAndSaveBankAccount}
          wrapperStyle={[
            styles.inputWrapper,
            !validBankAccount && { borderColor: Colors.unPaidColor },
          ]}
        />
      </View>
      <View style={styles.inputContainer}>
        ...
        <CustomInput
          ...
          onChange={(e: NativeSyntheticEvent<TextInputFocusEventData>) =>
            setMobilePay(e.nativeEvent.text)
          }
          onBlur={formatAndSaveMobilePay}
          wrapperStyle={[
            styles.inputWrapper,
            !validMobilePay && { borderColor: Colors.unPaidColor },
          ]}
        />
      </View>
    </CustomModal>
