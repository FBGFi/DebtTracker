...
export const DebtCard = (props: DebtCardProps) => {
  ...
  const countTotal = () =>
    state[props.debtId].items
      .reduce((prevSum, item) => prevSum + item.price, 0)
      .toFixed(2);

  const combineStyles = (
    baseStyle: StyleProp<any>,
    paidStyle: StyleProp<any>,
    unPaidStyle: StyleProp<any>,
  ) => {
    return isPaid(state, props.debtId, debtHoldersState)
      ? [baseStyle, paidStyle]
      : [baseStyle, unPaidStyle];
  };

  return (
    <TouchableCard
      onPress={() => props.viewDebt(props.debtId)}
      style={combineStyles(
        styles.debtCard,
        styles.paidDebt,
        styles.unPaidDebt,
      ...
      <Text
        ...
        {countTotal()} {state[props.debtId].currency}
      </Text>
    </TouchableCard>
  );
};
