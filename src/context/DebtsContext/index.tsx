...
export const getDebtsStateFromStorage = async (): Promise<
  TDebtsState | undefined
> => {
...
export function debtsReducer(state: TDebtsState, action: TAction): TDebtsState {
  switch (action.type) {
    ...
    case "updateDebtItemPrice":
      state[action.value.id].items[action.value.itemIndex].price =
        action.value.price;
      break;
    case "updateDebtItemDescription":
      state[action.value.id].items[action.value.itemIndex].description =
    ...
    case "removeDebtHolderFromDebt":
      state[action.value.id].debtHolders.splice(
        state[action.value.id].debtHolders.findIndex(
          (id) => id === action.value.debtHolderId,
        ),
        1,
      );
      break;
...
}>({ state: debtsInitialState, dispatch: () => {} });
...
export const useUpdateDebtItemDescription = () => {
  ...
  const updateDebtItemDescription = (
    debtId: string,
    itemIndex: number,
    description: string,
  ) => {
    if (!state[debtId]) return;
    dispatch({
      type: "updateDebtItemDescription",
      value: {
        id: debtId,
        itemIndex,
        description,
...
export const useAddDebtHolderToDebt = () => {
  ...
  const addDebtHolderToDebt = (debtId: string, debtHolderId: string) => {
    if (
      !debtHoldersState[debtHolderId] ||
      !state[debtId] ||
      state[debtId].debtHolders.includes(debtHolderId)
    ) {
      return;
    }
    dispatch({
      type: "addDebtHolderToDebt",
      value: {
        id: debtId,
        debtHolderId,
      },
